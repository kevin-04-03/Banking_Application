...
#include <dirent.h>
#include <ctype.h>
#include <sys/stat.h> // Required for mkdir on macOS/Linux
...
void createDatabaseFolder() {
    ...
    #else
        mkdir(DATABASE_FOLDER, 0777); // Standard macOS/Linux directory creation
    #endif
}

// List accounts available in database (Updated for macOS compatibility)
void listAccounts(){
    ...
    while ((entry = readdir(dir)) != NULL) {
        // Look for files ending with .txt extension
        ...
        if (dot && strcmp(dot, ".txt") == 0) {
            ...
            // Remove the extension for display purposes
...
void createBankAccount() {
    ...
    srand(time(0));
    do {
        account.account_number = (rand() % (1000000 - 10000 + 1)) + 10000;  
